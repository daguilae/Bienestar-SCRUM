...
{
    public partial class Frm_Consulta_Gastos : Form
    {
        LAPMRP logic = new LAPMRP();
        public Frm_Consulta_Gastos()
        {
            InitializeComponent();
        ...
        {
            OdbcDataReader mostrar = logic.consultarGastos();
            try
            {
                while (mostrar.Read())
                {
        ...
        private void Frm_Consulta_Gastos_Load(object sender, EventArgs e)
        {
            Dgv_consulta.Rows.Clear();
            MostrarConsulta();
        }
