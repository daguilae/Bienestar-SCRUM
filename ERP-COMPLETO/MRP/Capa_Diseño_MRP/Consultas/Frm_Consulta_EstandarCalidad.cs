...
{
    public partial class Frm_Consulta_EstandarCalidad : Form
    {
        LAPMRP logic = new LAPMRP();
        public Frm_Consulta_EstandarCalidad()
        {
            InitializeComponent();
        ...
        {
            OdbcDataReader mostrar = logic.consultarEstandar();
            try
            {
                while (mostrar.Read())
                {
        ...
        private void Frm_Consulta_EstandarCalidad_Load(object sender, EventArgs e)
        {
            Dgv_consulta.Rows.Clear();
            MostrarConsulta();
        }
